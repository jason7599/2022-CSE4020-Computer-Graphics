...
def display():
    ...
    T1=getTranslation(cow2wld1)
    T2=getTranslation(cow2wld2)
    ...
    T3 = T2 * abs(animTime) + T1 * (1-abs(animTime))
    
    delta=(R1.inv()*R2).as_rotvec()*abs(animTime)

    R3=R1*Rotation.from_rotvec(delta)

    #drawCow(cow2wld1);                                          # Draw cow.
    #drawCow(cow2wld2);                                          # Draw cow.
    #drawCow(matrix4(R3.as_matrix(), getTranslation(cow2wld2)))
    drawCow(matrix4(R3.as_matrix(), T3))
