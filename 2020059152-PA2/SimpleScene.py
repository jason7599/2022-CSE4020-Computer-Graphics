...
catmullRom = np.array([[-.5, 1.5, -1.5, .5],
              [1., -2.5, 2., -.5],
              [-.5, 0., .5, 0.],
              [0., 1., 0., 0.]])
...
def display():
    global cameraIndex, cow2wld,controlPoints,AnimState,animStartTime,animTime,cursorCow
    ...
    if AnimState:
        ...
        startIndex=math.floor(animTime)
        endIndex=startIndex+1

        nextCow=np.eye(4)

        t=animTime-startIndex
        T = np.array([t**3,t**2,t**1,t**0])
        points=[]
        for i in range(4):
            points.append(getTranslation(controlPoints[(startIndex+i-1)%6]))
        K = np.vstack((points[0], points[1], points[2], points[3]))
    
        setTranslation(nextCow, T @ catmullRom @ K)
        
        R=np.eye(3)
        direction=normalize(getTranslation(nextCow)-getTranslation(cursorCow))
        u=np.cross((0,1,0),direction)
        ...
        setRotation(nextCow,R)
        cursorCow=nextCow

        #setTranslation(cursorCow,T3)
        if startIndex < 18:
            drawCow(cursorCow,False)
        else:
...
def onMouseButton(window,button, state, mods):
    global controlPoints,isDrag, V_DRAG, H_DRAG,AnimState,animStartTime,animTime,initState,cursorCow #H_DRAG=1V_DRAG=2
    if AnimState: #no inputs taken when in AnimState
        return
    GLFW_DOWN=1;
    GLFW_UP=0;
    ...
    if isDrag: 
        ...
        if  isDrag==V_DRAG:
            ...
            if cursorOnCowBoundingBox:
                ...
                #p=Plane(np.array((0,1,0)),[0,yPos,0])
...
def main():
    ...
    width = 800;
    height = 600;
    window = glfw.create_window(width, height, "2020059152's Abbatoir", None, None)
    if not window:
        glfw.terminate()
        sys.exit(-1)
